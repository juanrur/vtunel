...
export async function GET (request: NextRequest): Promise<NextResponse> {
  ...
  const provider = authMethods.oauth2?.providers?.find((p: any) => p.name === 'github')
  if (!provider) {
    return new NextResponse('GitHub OAuth provider not configured', { status: 500 })
  }

  const githubUrl = new URL(provider.authURL)
  ...
  const store = cookies()
  store.set('oauth_github', JSON.stringify(state), {
    httpOnly: true,
    sameSite: 'lax',
    path: '/',
    maxAge: 600
